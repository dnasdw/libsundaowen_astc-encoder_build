...
mkdir build
cd build
cmake -DBUILD64=OFF -C "$rootdir/CMakeLists.txt" -DSHAREDLIB=ON -DCLI=OFF -DBUILD_TESTING=OFF -DCMAKE_INSTALL_PREFIX="$prefix" ..
...
cp -f "$tmpdir/$version/build/Source/"*.so "../target/lib/$target"
popd
rm -rf "$tmpdir"
